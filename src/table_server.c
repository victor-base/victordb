 ...
 *
 * This function scans all keys in the database and prints them in a readable format
 * with both hexadecimal and string representations when possible.
 *
 * @param core Pointer to the VictorTable database structure.
 */
void victor_table_debug_dump(VictorTable *core) {
    int ret;
    int found;
    uint64_t sz;

    if ((ret = kv_size(core->table, &sz)) != KV_SUCCESS) {
        log_message(LOG_ERROR, "Failed to read kv_size: %s", table_strerror(ret));
        return;
    }

    KVResult *results = malloc(sizeof(KVResult) * sz);
    
    if (!results) {
        log_message(LOG_ERROR, "Failed to allocate memory for debug dump");
        return;
    }
    
    kv_unsafe_lock(core->table);

    if ((ret = kv_unsafe_prefix_scan(core->table, "*", 1, results, sz, &found)) != KV_SUCCESS) {
        log_message(LOG_ERROR, "Failed kv_unsafe_prefix_scan: %s", table_strerror(ret));
        kv_unsafe_unlock(core->table);
        return;
    }
    
    printf("\n=== VictorDB Debug Key-Value Dump ===\n");
    printf("Database: %s\n", core->name);
    printf("Total keys found: %d\n", found);
    printf("====================================\n\n");
    
    for (int i = 0; i < found; i++) {
        KVResult *result = &results[i];
        
        // Print key in hex format
        printf("KEY[%d]: ", i + 1);
        for (int j = 0; j < result->klen; j++) {
            printf("%02x", ((unsigned char*)result->key)[j]);
        }
        
        // Print key as string if printable
        printf(" (");
        bool is_printable = true;
        for (int j = 0; j < result->klen; j++) {
            char c = ((char*)result->key)[j];
            if (c >= 32 && c <= 126) {
                printf("%c", c);
            } else {
                is_printable = false;
        ...
        for (int j = 0; j < result->vlen; j++) {
            printf("%02x", ((unsigned char*)result->value)[j]);
        }
        
        // Print value as string if printable
        printf(" (");
        is_printable = true;
        for (int j = 0; j < result->vlen; j++) {
            char c = ((char*)result->value)[j];
            if (c >= 32 && c <= 126) {
                printf("%c", c);
            } else {
                is_printable = false;
